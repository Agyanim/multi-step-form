    ...
    <div className='w-[94%]'>
      <ul className='bg-LightGray/40 w-[80%] m-auto p-4 rounded-md'>
        <li className='text-MarineBlue font-bold'>
          <span className='flex justify-between items-center  mb-1'>{selectedService.plan} {isYearly? "(Yearly)": "(Monthly)"} <span>${selectedService.price}{isYearly?"/yr":"/mo"}</span></span>
          <span className= ' text-CoolGray font-light cursor-pointer underline'> <Link to="/selectplan">Change</Link>  </span>
          </li>
